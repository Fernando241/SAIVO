                        ...
                        <tr class=" hover:border border-slate-500">
                            ...
                            <td class="text-center">
                                <form action="{{ route('cart.update', $id) }}" method="post">
                                    @csrf
                                    <input type="number" name="quantity" value="{{ $details['quantity'] }}" class="w-16"/>
                    ...
                    <tr class="text-center">
                        <td>
                            <form action="{{ route('cart.clear') }}" method="post">
                                @csrf
                                <button type="submit" class="inline-block px-6 text-sm font-medium text-red-500 bg-white hover:bg-red-200 rounded-md md:px-8 py-3">Vaciar Carrito</button>
                            </form>
                        </td>
                        <td>
                            <a href="{{ route('productos.index') }}" class="inline-block px-8 py-3 text-sm font-medium text-green-800 bg-white hover:bg-green-400 rounded-md">Seguir Comprando</a>
                        </td>
                        <td>
                        ...
                        <td>
                            <form action="{{ route('cart.checkout') }}" method="post">
                                @csrf
                                <button type="submit" class="inline-block px-8 py-3 text-sm font-medium text-white bg-green-600 hover:bg-green-800 rounded-md">Finalizar Compra</button>
                            </form>
                        </td>
                    </tr>
                </tfoot>
